...
public class BundleDescriptionBuilder {
	...
	private static final String[] DEFINED_EXPORT_PACKAGE_DIRECTIVES = {Constants.USES_DIRECTIVE, Constants.INCLUDE_DIRECTIVE, Constants.EXCLUDE_DIRECTIVE, BundleDescriptionBuilder.FRIENDS_DIRECTIVE, BundleDescriptionBuilder.INTERNAL_DIRECTIVE, Constants.MANDATORY_DIRECTIVE};
	private static final String[] DEFINED_IMPORT_PACKAGE_DIRECTIVES = {Constants.RESOLUTION_DIRECTIVE};
	private static final String[] DEFINED_PACKAGE_MATCHING_ATTRS = { Constants.BUNDLE_SYMBOLICNAME_ATTRIBUTE,
	...
	public static BundleDescriptionImpl createBundleDescription(Dictionary<String, String> manifest,
		...
		boolean jreBundle = "true".equals(manifest.get(BundleDescriptionBuilder.Eclipse_JREBUNDLE)); //$NON-NLS-1$
		...
		try {
			...
			if (manifestVersion >= 2) {
				String message = NLS.bind(StateMsg.MANIFEST_INVALID_HEADER_EXCEPTION, Constants.BUNDLE_VERSION, version);
		...
		result.setPlatformFilter(manifest.get(BundleDescriptionBuilder.ECLIPSE_PLATFORMFILTER));
		String[] brees = ManifestElement.getArrayFromList(manifest.get(BUNDLE_REQUIREDEXECUTIONENVIRONMENT));
		...
		ManifestElement[] provides = ManifestElement.parseHeader(BundleDescriptionBuilder.PROVIDE_PACKAGE, manifest.get(BundleDescriptionBuilder.PROVIDE_PACKAGE));
		...
		String[][] genericAliases = getGenericAliases();
	...
	static BundleSpecification createRequiredBundle(ManifestElement spec) {
		...
		result.setExported(Constants.VISIBILITY_REEXPORT.equals(spec.getDirective(Constants.VISIBILITY_DIRECTIVE)) || "true".equals(spec.getAttribute(BundleDescriptionBuilder.REPROVIDE_ATTRIBUTE))); //$NON-NLS-1$
		result.setOptional(Constants.RESOLUTION_OPTIONAL.equals(spec.getDirective(Constants.RESOLUTION_DIRECTIVE)) || "true".equals(spec.getAttribute(BundleDescriptionBuilder.OPTIONAL_ATTRIBUTE))); //$NON-NLS-1$
	...
	static void addExportPackages(ManifestElement exportPackage, List<ExportPackageDescription> allExports, boolean strict) {
		...
		for (String exportName : exportNames) {
			...
			if (strict && "true".equals(exportPackage.getDirective(BundleDescriptionBuilder.INTERNAL_DIRECTIVE))) //$NON-NLS-1$
				continue;
			...
			result.setDirective(BundleDescriptionBuilder.FRIENDS_DIRECTIVE, ManifestElement.getArrayFromList(exportPackage.getDirective(BundleDescriptionBuilder.FRIENDS_DIRECTIVE)));
			result.setDirective(BundleDescriptionBuilder.INTERNAL_DIRECTIVE, Boolean.valueOf(exportPackage.getDirective(BundleDescriptionBuilder.INTERNAL_DIRECTIVE)));
	...
	static List<GenericSpecification> convertBREEs(String[] brees, List<GenericSpecification> result) throws BundleException {
		...
		spec.setType(BundleDescriptionBuilder.OSGI_EE_NAMESPACE);
		try {
			Filter filter = FrameworkUtil.createFilter(filterSpec);
			spec.setMatchingFilter(filter);
			String name = getPrimaryKeyValue(filter, spec.getType());
			if (name != null)
				spec.setName(name);
	...
	private static String getPrimaryKeyValue(Filter filter, String key) {
		try {
			Method method = filter.getClass().getMethod("getPrimaryKeyValue", String.class); //$NON-NLS-1$
			...
			for (String namespace : namespaces) {
				...
				if (filterSpec != null) {
					try {
						Filter filter = FrameworkUtil.createFilter(filterSpec);
						spec.setMatchingFilter(filter);
						String name = getPrimaryKeyValue(filter, namespace);
						if (name != null)
							spec.setName(name);
					} catch (InvalidSyntaxException e) {
						String message = NLS.bind(StateMsg.MANIFEST_INVALID_HEADER_EXCEPTION, Constants.REQUIRE_CAPABILITY, element.toString());
		...
		for (ManifestElement equinoxRequire : equinoxRequires) {
			...
			for (String genericName : genericNames) {
				...
				try {
					...
					String message = NLS.bind(StateMsg.MANIFEST_INVALID_HEADER_EXCEPTION, GENERIC_REQUIRE, equinoxRequire.toString());
		...
		for (int i = 0; i < length; i++) {
			...
			for (String packageName : packageNames) {
				if (!export && !dynamic && packages.contains(packageName)) {
					String message = NLS.bind(StateMsg.MANIFEST_INVALID_HEADER_EXCEPTION, headerKey, elements[i].toString());
			...
			if (version != null) {
				String specVersion = elements[i].getAttribute(PACKAGE_SPECIFICATION_VERSION);
				if (specVersion != null && !specVersion.equals(version))
					throw new BundleException(NLS.bind(StateMsg.HEADER_VERSION_ERROR, Constants.VERSION_ATTRIBUTE,
							PACKAGE_SPECIFICATION_VERSION), BundleException.MANIFEST_ERROR);
			}
			...
			if (directiveKeys != null) {
				while (directiveKeys.hasMoreElements()) {
					...
					if (directives.length > 1) {
						String message = NLS.bind(StateMsg.MANIFEST_INVALID_HEADER_EXCEPTION, headerKey, element.toString());
	...
	private static void checkExtensionBundle(String headerKey, ManifestElement[] elements) throws BundleException {
		...
		if (!hostName.equals(Constants.SYSTEM_BUNDLE_SYMBOLICNAME) && !hostName.equals(BundleDescriptionBuilder.NAME)) {
			String message = NLS.bind(StateMsg.MANIFEST_INVALID_HEADER_EXCEPTION, headerKey, elements[0].toString());
