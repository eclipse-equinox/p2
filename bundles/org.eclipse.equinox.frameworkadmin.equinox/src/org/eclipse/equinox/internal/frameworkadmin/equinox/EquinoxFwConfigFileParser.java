...
public class EquinoxFwConfigFileParser {
	...
	private void readBundlesList(Manipulator manipulator, URI osgiInstallArea, Properties props)
			throws NumberFormatException {
		ConfigData configData = manipulator.getConfigData();

		BundleInfo[] fwExtensions = parseBundleList(manipulator,
				props.getProperty(EquinoxConstants.PROP_FW_EXTENSIONS));
	...
	private void writeBundlesList(File fwJar, Properties props, BundleInfo[] bundles) {
		...
		for (BundleInfo bundle : bundles) {
			...
			boolean isFrameworkExtension = fragmentHost != null
					&& (fragmentHost.startsWith(EquinoxConstants.FW_SYMBOLIC_NAME)
							|| fragmentHost.startsWith(Constants.SYSTEM_BUNDLE_SYMBOLICNAME));
	...
	public void readFwConfig(Manipulator manipulator, File inputFile) throws IOException, URISyntaxException {
		...
		Properties sharedConfigProperties = getSharedConfiguration(
				ParserUtils.getOSGiInstallArea(Arrays.asList(manipulator.getLauncherData().getProgramArgs()), props,
		...
		// URI rootURI = launcherData.getLauncher() != null ?
		// launcherData.getLauncher().getParentFile().toURI() : null;
	...
	private File readFwJarLocation(ConfigData configData, LauncherData launcherData, Properties props)
		...
		if (props.getProperty(EquinoxConstants.PROP_OSGI_FW) != null) {
			...
			absoluteFwJar = URIUtil.makeAbsolute(
					FileUtils.fromFileURL(props.getProperty(EquinoxConstants.PROP_OSGI_FW)),
					ParserUtils.getOSGiInstallArea(Arrays.asList(launcherData.getProgramArgs()), props, launcherData)
							.toURI());
		...
		props.setProperty(EquinoxConstants.PROP_OSGI_FW,
				FileUtils.toFileURL(URIUtil.makeRelative(launcherData.getFwJar().toURI(),
						ParserUtils.getOSGiInstallArea(Arrays.asList(launcherData.getProgramArgs()),
								configData.getProperties(), launcherData).toURI())));
	}
	...
	private File findSharedConfigIniFile(File base, String sharedConfigurationArea) {
		...
		try {
			...
			Log.warn(NLS.bind(Messages.log_shared_config_url, sharedConfigurationArea));
			return null;
		}
		...
		if (!sharedConfigurationURL.getPath().startsWith("/")) { //$NON-NLS-1$
			if (!rootURL.getProtocol().equals(sharedConfigurationURL.getProtocol())) {
				Log.warn(NLS.bind(Messages.log_shared_config_relative_url, rootURL.toExternalForm(),
		...
		if (!sharedConfigurationFolder.isDirectory()) {
			Log.warn(NLS.bind(Messages.log_shared_config_file_missing, sharedConfigurationFolder));
			return null;
		}
	...
	public void saveFwConfig(BundleInfo[] bInfos, Manipulator manipulator, boolean backup, boolean relative)
			throws IOException {// {
		ConfigData configData = manipulator.getConfigData();
		LauncherData launcherData = manipulator.getLauncherData();
		...
		// TODO This can't be done before because of the previous calls to
		// appendProperties

		if (configProps == null || configProps.size() == 0) {
			Log.warn(this, "saveFwConfig() ", Messages.log_configProps); //$NON-NLS-1$
			return;
		}
	...
	private void filterPropertiesFromSharedArea(Properties configProps, Manipulator manipulator) {
		LauncherData launcherData = manipulator.getLauncherData();
		// Remove from the config file that we are about to write the properties that
		// are unchanged compared to what is in the base
		Properties sharedConfigProperties = getSharedConfiguration(
				ParserUtils.getOSGiInstallArea(Arrays.asList(launcherData.getProgramArgs()), configProps, launcherData),
				configProps.getProperty(EquinoxConstants.PROP_SHARED_CONFIGURATION_AREA));
		if (sharedConfigProperties == null)
	...
	private void rememberSharedConfigurationTimestamp(Properties configProps, Manipulator manipulator, File folder)
			throws IOException {
		LauncherData launcherData = manipulator.getLauncherData();
		// Remove from the config file that we are about to write the properties that
		// are unchanged compared to what is in the base
