...
public class JREAction extends AbstractPublishingAction {
	...
	public static IArtifactDescriptor createJREData(File jreLocation, IPublisherResult results) {
		...
		cu.setHost(new RequiredCapability[] {MetadataFactory.createRequiredCapability(IInstallableUnit.NAMESPACE_IU_ID, id, new VersionRange(version, true, MetadataGeneratorHelper.versionMax, true), null, false, false)});
		cu.setProperty(IInstallableUnit.PROP_TYPE_FRAGMENT, Boolean.TRUE.toString());
		cu.setCapabilities(new ProvidedCapability[] {MetadataGeneratorHelper.createSelfCapability(configId, version)});
		...
		IArtifactKey key = new ArtifactKey(MetadataGeneratorHelper.BINARY_ARTIFACT_CLASSIFIER, id, version);
		...
		return MetadataGeneratorHelper.createArtifactDescriptor(key, jreLocation);
	}
	...
	private static ProvidedCapability[] generateJRECapability(String installableUnitId, Version installableUnitVersion, InputStream profileStream) {
		...
		try {
			...
			for (int i = 1; i <= jrePackages.length; i++) {
				exportedPackageAsCapabilities[i] = MetadataFactory.createProvidedCapability(MetadataGeneratorHelper.CAPABILITY_NS_JAVA_PACKAGE, jrePackages[i - 1].getValue(), null);
			}
