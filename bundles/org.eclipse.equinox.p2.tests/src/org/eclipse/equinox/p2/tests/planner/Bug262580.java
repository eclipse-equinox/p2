...
public class Bug262580 extends AbstractProvisioningTest {
	public void testRevertFeaturePatch() {
		...
		ProvisioningPlan plan = planner.getRevertPlan(currentProfile, revertProfile, new ProvisioningContext(new URI[0]), getMonitor());
		assertTrue(plan.getStatus().isOK());
	}

	public void testRevertFeaturePatchWithNullContext() {
		...
		ProvisioningPlan plan = planner.getRevertPlan(currentProfile, revertProfile, null, getMonitor());
		assertTrue(plan.getStatus().isOK());
	}
}
